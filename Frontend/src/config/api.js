...
export const getBaseURL = () => {
  ...
  // Default: use server URL (API đã chạy trên server)
  ...
  if (import.meta.env.DEV) {
    ...
    // Proxy đã được cấu hình trong vite.config.js: /v1 → http://localhost:5000
    return '';
...
if (import.meta.env.DEV) {
  console.log('[API Config]', {
    ...
    API_URL: API_URL,
    Backend: API_BASE_URL ? `Direct: ${API_BASE_URL}` : 'Proxy: http://localhost:5000'
  });
}
